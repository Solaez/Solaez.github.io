...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 require '../../php/baseDatos.php';
    ...
    $sql = "UPDATE productos SET nombre='$nombre', categorias='$categoria', descripcion='$descripcion', tipo='$tipo', lugar='$ubicacion', precio='$precio', precio2='$precio_promocion', estado='$promocion' WHERE id_producto=$id_producto";

    if ($conn->query($sql) === TRUE) {
        // Manejar las imágenes
        $uploadDir = '/1pagina civil/sitios/productoCivil/';
        for ($i = 1; $i <= 4; $i++) {
            $imagen = 'imagen' . $i;
            if (isset($_FILES[$imagen]) && $_FILES[$imagen]['error'] == UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($tmp_name, $_SERVER['DOCUMENT_ROOT'] . $uploadFile)) {
                    $sql = "UPDATE productos SET $imagen='$uploadFile' WHERE id_producto=$id_producto";
                    if ($conn->query($sql) !== TRUE) {
